/* global React */
// SCORM export — builds a self-contained SCORM 1.2 package as a .zip

// Minimal ZIP writer (STORE only, no compression) — sufficient for SCORM
function crc32(data) {
  let c;
...
/* ---- SCORM 1.2 manifest + wrapper HTML ---- */
...
function standaloneHTML(set, variant) {
  ...
  // Minimal student-mode HTML, includes SCORM API finder + mark complete
  return `<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="utf-8"/>
...
function standaloneCSS() {
  // Compact student-mode CSS — no editor chrome.
  return `@import url('https://fonts.googleapis.com/css2?family=Oswald:wght@700&display=swap');
...
.btn { background:var(--wheat); color:var(--blue); border:none; padding:10px 18px; border-radius:999px; font-weight:700; font-size:14px; cursor:pointer; }
.btn:disabled { opacity:.4; cursor:not-allowed; }
.ghost { background:transparent; border:none; color:var(--black); font-weight:700; cursor:pointer; padding:10px; }
...
.title { font-size:clamp(28px, 4vw, 44px); margin:0 0 10px; font-weight:700; line-height:1.1; }
.sub { color:#303b66; font-size:16px; line-height:1.5; margin:0 0 22px; }
.start { background:var(--blue); color:#fff; padding:12px 28px; border:none; border-radius:999px; font-weight:700; font-size:15px; cursor:pointer; }
...
.done h3 { font-size:clamp(24px, 3vw, 36px); margin:0 0 10px; }
...
function standaloneJS() {
  ...
  function render(){
    ...
    if(pos === -1){
      ...
      html += cardView(set.cards[pos], pos);
    }
    html += '</div>';
    root.innerHTML = html;
    bind();
      ...
      '<div class="flip '+(side==='back'?'flipped':'')+'"><div class="inner">'+
        ...
        '<div class="face back" tabindex="'+(side==='back'?0:-1)+'" role="button" aria-pressed="'+(side==='back')+'" data-act="flip">'+
          '<div class="scroll" data-scroll>'+
            ...
            '<div class="a" data-ans>'+esc(c.back)+'</div>'+
          '</div>'+
        '</div>'+
      '</div></div>'+
      '<div class="row">'+
...
async function exportScorm(set, variant) {
  const files = [
    { name: 'imsmanifest.xml', data: strToU8(imsmanifest(set.title)) },
    { name: 'index.html', data: strToU8(standaloneHTML(set, variant)) },
...
window.exportScorm = exportScorm;
