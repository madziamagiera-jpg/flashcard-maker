/* global React, ReactDOM, Icon, Toast, Player, EditorPanel, exportScorm */
...
// --- Default / template content: California flashcards ---
const TEMPLATE_SET = {
  id: 'template',
...
const Library = ({ sets, onOpen, onNew, onDuplicate, onDelete, onOpenTemplate }) => (
  <div className="library">
    <div className="library__header">
      ...
      <button className="iconbtn solid" onClick={onNew}><Icon name="plus" size={14}/> New set</button>
    </div>
    ...
    <div className="set-grid">
      ...
      <div className="set-card" style={{borderColor:'var(--wset-heritage-blue)'}}>
        <div
          ...
          style={{ backgroundImage: `url(assets/default-hero.jpeg)` }}
        >
        ...
        <div className="set-card__body">
          ...
          <div className="set-card__actions">
            <button className="primary" onClick={onOpenTemplate}><Icon name="copy" size={12}/> Use template</button>
          </div>
        </div>
      </div>
      ...
      {sets.map(s => (
        <div key={s.id} className="set-card">
          ...
          >
            <span className="chip-count">{s.cards.length} cards</span>
          </div>
          <div className="set-card__body">
            <h3>{s.title || 'Untitled set'}</h3>
            <div className="meta">Edited {new Date(s.updated).toLocaleDateString()}</div>
            <div className="set-card__actions">
              <button className="primary" onClick={() => onOpen(s.id)}><Icon name="edit" size={12}/> Open</button>
              <button onClick={() => onDuplicate(s.id)}><Icon name="copy" size={12}/> Duplicate</button>
              <button className="danger" onClick={() => onDelete(s.id)}><Icon name="trash" size={12}/> Delete</button>
            </div>
          </div>
        </div>
      ))}
...
// -------- App --------
const App = () => {
  const [sets, setSetsState] = React.useState(() => loadSets() || []);
  const [screen, setScreen] = React.useState('library'); // library | editor
  const [activeId, setActiveId] = React.useState(null);
  const [draft, setDraft] = React.useState(null); // current working set
  ...
  const setTweaks = (t) => { setTweaksState(t); localStorage.setItem(TWEAKS_KEY, JSON.stringify(t)); };
  ...
  const openNew = () => {
    const s = newBlankSet();
    setDraft(s); setActiveId(null); setDirty(true); setSelected(0); setScreen('editor'); setMode('edit');
  };
  const openTemplate = () => {
    // Use template as a STARTING POINT for a new set (template itself is never overwritten)
    const copy = { ...TEMPLATE_SET, id: 's' + Date.now(), title: TEMPLATE_SET.title + ' (copy)', updated: Date.now() };
    setDraft(copy); setActiveId(null); setDirty(true); setSelected(0); setScreen('editor'); setMode('edit');
  };
  ...
  const save = () => {
    ...
    let next;
    if (activeId) {
      next = sets.map(s => s.id === activeId ? stamped : s);
    } else {
      next = [...sets, stamped];
  ...
  // Ensure preview shows the currently-selected card by default
  React.useEffect(() => {
    if (mode === 'preview') {
      // nothing; Player manages its own pos
    }
  }, [mode]);
      ...
      <header className="topbar">
        ...
        {screen === 'editor' && (
          <>
            ...
            <button className="iconbtn" onClick={save}><Icon name="save" size={14}/> Save</button>
      ...
      {screen === 'editor' && draft && (
        <div className="editor">
          ...
          <PreviewStage
            ...
            key={mode /* re-mount when switching modes */}
          />
        </div>
      )}
...
// A thin Player wrapper that starts on the selected card in edit mode
const PreviewStage = ({ set, variant, framed, initialPos }) => {
  // Use a keyed Player to reset per-mount; the Player itself starts at -1 (title)
  // but we want edit-mode to jump straight to the selected card.
  return <Player set={set} variant={variant} framed={framed} initialPos={initialPos}/>;
};
