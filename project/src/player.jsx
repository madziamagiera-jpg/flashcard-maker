/* global React, Icon, useAdaptiveFont */
// Flashcard player component — title / cards / done
  ...
  >
    <div className="title-card__box">
      ...
      <div className="title-card__tip">
        Tip: use <span className="kbd">←</span> <span className="kbd">→</span> to navigate and <span className="kbd">Space</span> to flip.
      </div>
    </div>
  </div>
);
...
/* Player — handles title→cards→done state */
const Player = ({ set, variant = 'default', framed = true, onExit, initialPos = -1 }) => {
  // -1 = title; >= total = done; otherwise = card index
...
Object.assign(window, { Player, Flashcard, TitleCard, DoneCard });
